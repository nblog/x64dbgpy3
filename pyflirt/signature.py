...
'''
from: https://github.com/mokhdzanifaeq/pyflirt/blob/master/core/signature.py
...
''' https://github.com/rizinorg/rizin/blob/v0.4.0/librz/signature/flirt.c#L172 '''
class header_v10(Structure):
	_fields_ = [
		("unknown", c_ushort)
	]

class idasig:
	def __init__(self, sign:bytes, dump:bool=False):
		self.binary = sign
		self.dump = dump
	...
	def parse_header(self):
		...
		if self.header["magic"] != b"IDASGN":
			raise Exception("not a signature file")
		if self.header["version"] < 5 or self.header["version"] > 10:
		...
		if self.header["version"] >= 6:
			...
			if self.header["version"] >= 10:
				v10 = header_v10.from_buffer_copy(self.binary[self.skip:])
				self.skip += sizeof(header_v10)
				self.header["unknown"] = v10.unknown
	...
	def parse_tree(self, node):
		...
		for i in range(branches):
			...
			for j in range(child_node["length"]):
		...
		while True:
			...
			while True:
				...
				if flags & self.flag["parse"]["READ_REFERENCED_FUNCTIONS"]:
					...
					for i in range(n_referenced_functions):
						referenced_function = {}
	...
	def dump_header(self):
		...
		print ("file_types :")
		for file_type, fflag in self.flag["file_types"].items():
			if fflag & self.header["file_types"]:
				print (file_type)
		print ("(0x%X)" % self.header["file_types"])
		print ("os_types :")
		for os_type, fflag in self.flag["os_types"].items():
			if fflag & self.header["os_types"]:
				print (os_type)
		print ("(0x%X)" % self.header["os_types"])
		print ("app_types :")
		for app_type, fflag in self.flag["app_types"].items():
			if fflag & self.header["app_types"]:
				print (app_type)
		print ("(0x%X)" % self.header["app_types"])
		print ("features :")
		for feature, fflag in self.flag["features"].items():

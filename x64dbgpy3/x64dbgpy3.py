...
class dbgLogging:
    ...
    def logputs(*values) -> None:
        return X64DBGCALL.x64dbg_call(
            FUNCTION_NAME(dbgLogging), ["".join([str(i) for i in values])]
        )
...
class dbgAssembler:
    ...
    def AssembleEx(addr: ptr_t, instruction: str, fillnop: bool = True) -> str:
        res = X64DBGCALL.x64dbg_call(
            FUNCTION_NAME(dbgAssembler), [addr, instruction, fillnop]
        )
        return str(res)
...
class dbgLabel:
    ...
    @staticmethod
    def Set(
        addr: ptr_t, label: str, manual: bool = False, temporary: bool = False
    ) -> bool:
        res = X64DBGCALL.x64dbg_call(
            FUNCTION_NAME(dbgLabel), [addr, label, manual, temporary]
        )
        return bool(res)
...
class dbgFunction:
    ...
    @staticmethod
    def Add(
        start: ptr_t, end: ptr_t, manual: bool = False, instructionCount: size_t = 0
    ) -> bool:
        res = X64DBGCALL.x64dbg_call(
            FUNCTION_NAME(dbgFunction), [start, end, manual, instructionCount]
        )
        return bool(res)
...
class dbgArgument:
    ...
    def Add(
        ...
        res = X64DBGCALL.x64dbg_call(
            FUNCTION_NAME(dbgArgument), [start, end, manual, instructionCount]
        )
        return bool(res)
...
class dbgBreakpoint:
    ...
    def GetBreakpointList(
        bpxtype: DBGBREAKPOINTINFO.BPXTYPE = DBGBREAKPOINTINFO.BPXTYPE.bp_none,
    ) -> list[DBGBREAKPOINTINFO]:
    ...
    class DBGHARDWARETYPE(IntEnum):
        HardwareAccess, HardwareWrite, HardwareExecute = 0, 1, 2

        def __str__(self):
            return {0: "ACCESS", 1: "WRITE", 2: "EXECUTE"}.get(
                self.value, "UNKNOWN"
            ).lower()
    ...
    class DBGTHREADALLINFO(BaseModel):
        class FILETIME(BaseModel):
            ...
            def __str__(self):
                ...
                return filetime_to_datetime(filetime).strftime("%Y-%m-%d %H:%M:%S.%f")[
                    :-3
                ]
...
class dbgMemory:
    ...
    def Write(addr: ptr_t, data: bytes) -> bool:
        res = X64DBGCALL.x64dbg_call(
            FUNCTION_NAME(dbgMemory), [addr, RequestBuffer.serialize(data)]
        )
        return bool(res)
...
class dbgRegister:
    ...
    class DBGFLAGENUM(IntEnum):
        ...
        ZF, OF, CF, PF, SF, TF, AF, DF, IF = 0, 1, 2, 3, 4, 5, 6, 7, 8
    ...
    class DBGREGISTERENUM_DEBUG:
        ...
        DR0, DR1, DR2, DR3, DR6, DR7 = map(int, range(0, 6))
    ...
    class DBGREGISTERENUM(
        DBGREGISTERENUM_DEBUG, DBGREGISTERENUM_WIN32, DBGREGISTERENUM_WIN64, IntEnum
    ):
    ...
    class DBGREGDUMP(BaseModel):
        ...
        class REGISTERCONTEXT(BaseModel):
            ...
            MxCsr: int
            XmmRegisters: list[
                "dbgRegister.DBGREGDUMP.XMMREGISTER"
            ]  # std::array<XMMREGISTER_WRAPPER, 16/8>
            YmmRegisters: list[
                "dbgRegister.DBGREGDUMP.YMMREGISTER"
            ]  # std::array<YMMREGISTER_WRAPPER, 16/8>

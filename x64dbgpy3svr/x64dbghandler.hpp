...
namespace x64dbgSvrUtil
{
	...
	{
		static constexpr size_t MAX_BUFFER_SIZE = 100 * 1024 * 1024; // 100MB 限制
		...
		static std::string Serialize(const std::vector<uint8_t>& buffer)
		{
			if (buffer.empty())
			{
				throw std::invalid_argument("Cannot serialize empty buffer");
			}
			...
			std::vector<uint8_t> compressed(static_cast<size_t>(maxCompressedSize) + hdr_size);
			*reinterpret_cast<uint32_t*>(compressed.data()) = static_cast<uint32_t>(buffer.size());
			...
			if (compressedSize <= 0)
			{
				throw std::runtime_error(fmt::format("Compression failed with error code: {}", compressedSize));
			}

			compressed.resize(static_cast<size_t>(compressedSize) + hdr_size);

			size_t base64Len = simdutf::base64_length_from_binary(compressed.size());
			...
			return std::string(base64buffer.data(), written);
		}

		static std::vector<uint8_t> Deserialize(const std::string& base64buffer)
		{
			if (base64buffer.empty())
			{
				throw std::invalid_argument("Cannot deserialize empty base64 string");
			}

			size_t maxBinaryLen = simdutf::maximal_binary_length_from_base64(
				base64buffer.data(), base64buffer.size());

			std::vector<char> compressed(maxBinaryLen);
			...
			{
				throw std::runtime_error(fmt::format("Base64 decoding failed with error code: {}",
			...
			// 边界检查
			if (compressed.size() < hdr_size)
			{
				throw std::runtime_error(fmt::format(
					"Compressed data too small: {} bytes (minimum: {} bytes)",
					compressed.size(), hdr_size));
			}
			...
			if (originalSize == 0)
			{
				throw std::runtime_error("Invalid original size: 0");
			}

			if (originalSize > MAX_BUFFER_SIZE)
			{
				throw std::runtime_error(fmt::format(
					"Original size {} exceeds maximum allowed size {}",
					originalSize, MAX_BUFFER_SIZE));
			}
			...
			{
				throw std::runtime_error(fmt::format(
					"Decompression failed with error code: {}", decompressedSize));
			}

			if (static_cast<uint32_t>(decompressedSize) != originalSize)
			{
				throw std::runtime_error(fmt::format(
...
{
	auto FindPattern(const std::string& pattern)
	{
		return nlohmann::json();
	}
};
